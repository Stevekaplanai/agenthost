...
test("buildEnvSecrets rejects a repo not present in --repos", () => {
  assert.throws(
    () => buildEnvSecrets(["a/one"], ["b/two:FOO=bar"]),
    /must reference a repo listed in --repos/,
  );
});
...
test("buildEnvSecrets never echoes malformed input that may contain a secret", () => {
  const sentinel = "malformed-secret-must-not-echo";
  assert.throws(
    () => buildEnvSecrets(["a/one"], [`not-valid-${sentinel}`]),
    (error) => {
