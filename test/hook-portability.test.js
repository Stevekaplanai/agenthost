...
test("pack.mjs rejects a top-level --include symlink that resolves outside home", (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agenthost-include-boundary-"));
  ...
  fs.writeFileSync(path.join(outside, "outside-marker.txt"), "must not migrate\n");
  fs.symlinkSync(outside, path.join(home, "Vault"), process.platform === "win32" ? "junction" : "dir");

  const res = spawnSync(process.execPath, [PACK_MJS, "--out", out, "--dry-run", "--include", "Vault"], {
    env: { ...process.env, HOME: home, USERPROFILE: home }, encoding: "utf8",
  });
  assert.notEqual(res.status, 0, "an include resolving outside home must fail closed");
  assert.match(res.stderr, /include.*resolves outside.*home/i);
  assert.ok(!fs.existsSync(path.join(out, "staging", "Vault", "outside-marker.txt")),
    "content outside home must never be staged through a symlink or junction");
});
