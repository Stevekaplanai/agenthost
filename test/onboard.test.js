...
test("buildIncludeProposals handles Windows homes without depending on the test runner OS", () => {
  const home = "C:\\Users\\Steve";
  const { proposals, skipped } = buildIncludeProposals({
    home,
    vaultPaths: ["C:\\Users\\Steve\\Obsidian\\SecondBrain"],
    hookRefs: ["Projects/operator-brain/capture.py"],
    statPath: statFrom({
      "C:\\Users\\Steve\\Obsidian\\SecondBrain": "dir",
      "C:\\Users\\Steve\\Projects\\operator-brain\\capture.py": "file",
    }),
  });
  assert.deepEqual(proposals.map((p) => p.include), ["Obsidian/SecondBrain", "Projects/operator-brain"]);
  assert.deepEqual(skipped, []);
...
test("buildIncludeProposals handles forward-slash Windows UNC homes", () => {
  const home = "//server/share/Steve";
  const { proposals, skipped } = buildIncludeProposals({
    home,
    vaultPaths: ["//server/share/Steve/Vault", "//server/other/Vault"],
    statPath: statFrom({
      "\\\\server\\share\\Steve\\Vault": "dir",
      "\\\\server\\other\\Vault": "dir",
    }),
  });
...
test("buildIncludeProposals rejects hook refs that traverse outside home", () => {
  const { proposals, skipped } = buildIncludeProposals({
    home: "/home/steve",
    hookRefs: ["../outside/secret.py"],
    statPath: statFrom({ "/home/outside/secret.py": "file" }),
  });
  assert.deepEqual(proposals, []);
  assert.equal(skipped.length, 1);
  assert.match(skipped[0].reason, /outside your home directory/);
