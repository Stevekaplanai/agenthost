...
// 8080). AGENTHOST_DEV_WRAP points at a portable Node fake that echoes its
// prompt; a prompt containing SLOW waits first, which is how the tests hold the
// busy slot.
...
before(async () => {
  ...
  const charterFile = path.join(HOME, "test-charter.md");
  fs.writeFileSync(charterFile, "Test charter for the Windows spawn fixture.\n");
  const fakeBin = path.join(HOME, "fake-claude.mjs");
  ...
  // safely embedded. A prompt containing SLOW holds the busy slot for 2s. The
  // gate's existing dev wrapper seam launches this with process.execPath, so
  // the same fake works on Windows and Linux without a shell.
  fs.writeFileSync(fakeBin, [
    ...
    'process.stdout.write(JSON.stringify({type:"stream_event",event:{type:"content_block_delta",delta:{type:"text_delta",text:"echo:"+prompt}}})+"\\n");',
  ...
  gate = spawn("node", [GATE], {
    env: {
      ...
      AGENTHOST_DEV_WRAP: fakeBin,
      AGENT_CHARTER_FILE: charterFile,
      GATE_PORT: "0",
    },
...
test("busy slot: second message gets an immediate queued status, then its answer when the slot frees", async () => {
  ...
  assert.ok(statusAt > 0, `a status event was emitted; received ${buf.slice(0, 1200)}`);
...
test("free slot: no status event is emitted", async () => {
  ...
  assert.equal(streamedText(events), "echo:fast question", `received ${JSON.stringify(events).slice(0, 1200)}`);
});
...
test("brain: a full question greps keywords, not the sentence literal (task #34)", async () => {
  ...
  assert.match(prompt, /attribyte\.md/, `grep hit carries the file name; received ${JSON.stringify(events).slice(0, 1200)}`);
