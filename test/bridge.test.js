...
test("port validation accepts real ports and rejects junk", () => {
  ...
  for (const bad of ["0", "65536", "abc", "", undefined, "12.5"]) {
    assert.throws(() => validateBridgePort(bad), /invalid bridge port/i);
  }
});

test("bridge waits for secret staging before deploying staged secrets", async () => {
  const calls = [];
  let finishStage;
  const staged = new Promise((resolve) => { finishStage = resolve; });
  const pending = stageBridgeSecrets(
    "fixture-app",
    { BRIDGE_URL: "https://fixture.example" },
    {
      stageSecrets: () => {
        calls.push("stage");
        return staged;
  ...
  finishStage();
  await pending;
  assert.deepEqual(calls, ["stage", "deploy"]);
});

test("bridge does not deploy when async secret staging rejects", async () => {
  let deployed = false;
  await assert.rejects(
    () => stageBridgeSecrets(
      ...
      { BRIDGE_URL: "https://fixture.example" },
      {
        stageSecrets: async () => { throw new Error("staging failed safely"); },
        run: () => {
          deployed = true;
          return { code: 0, stdout: "", stderr: "" };
        },
    ...
    /staging failed safely/,
  );
  assert.equal(deployed, false);
});

test("bridge rejects every nonzero Fly deploy result even when its error says already", async () => {
  for (const output of [
