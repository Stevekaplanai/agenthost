...
// "full" migrates the complete supported harness. "safe" automatically
// migrates only the credential-free subset and exposes an explicit opt-out.
export const AGENTS = [
  { key: "claude", name: "Claude Code", dir: ".claude", support: "full" },
  { key: "hermes", name: "Hermes", dir: ".hermes", support: "safe", optOut: "--no-hermes" },
  { key: "codex", name: "Codex", dir: ".codex", support: "safe", optOut: "--no-codex" },
  { key: "openclaw", name: "OpenClaw", dir: ".openclaw", support: "safe", optOut: "--no-openclaw" },
];
...
export function describeAgent(a) {
  ...
  switch (a.support) {
    ...
    case "safe":
      return `${a.name}: detected -- safe files migrate automatically (${a.optOut} to skip)`;
    default:
      return `${a.name}: detected`;
  }
}
