...
//     every real endpoint). The CLI refuses to bridge without --token-env unless
...
export function validateBridgePort(raw) {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    throw new Error(
      "Invalid bridge port. Usage: agenthost bridge <port> [--app <name>] [--token-env]",
    );
  }
  return port;
}
...
function ts(args) {
  const res = spawnSync(tailscalePath(), args, {
    encoding: "utf8",
    env: minimalChildEnv(),
  });
  if (res.error) {
...
export async function stageBridgeSecrets(
  app,
  secrets,
  { stageSecrets = fly.stageSecrets, run = fly.run } = {},
) {
  await stageSecrets(app, secrets);
  const deployed = run(["secrets", "deploy", "-a", app]);
  if (deployed.code !== 0) {
    throw new Error(
      `bridge secrets staged but could not be deployed to the box:\n`
...
export async function bridgeCommand(flags) {
  ...
  if (!token && !flags["no-token"]) {
    throw new Error(
      ...
      "Pass --token-env (reads AGENTHOST_BRIDGE_TOKEN or prompts with input hidden, then stores it as BRIDGE_TOKEN),\n" +
  ...
  await stageBridgeSecrets(
    app,
    { BRIDGE_URL: url, ...(token ? { BRIDGE_TOKEN: token } : {}) },
  );
