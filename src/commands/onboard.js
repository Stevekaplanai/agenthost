...
function pathApiFor(home) {
  return /^[A-Za-z]:[\\/]/.test(home) || home.startsWith("\\\\") || home.startsWith("//")
...
export function buildIncludeProposals({ home, vaultPaths = [], hookRefs = [], statPath = defaultStat }) {
  ...
  const homeAbs = pathApi.resolve(home);
  const proposals = [];
  const skipped = [];
  const seen = new Set();
  const propose = (rel, reason) => {
    const key = rel.split(pathApi.sep).join("/");
  ...
  for (const vp of vaultPaths) {
    const abs = pathApi.resolve(vp);
    const rel = pathApi.relative(homeAbs, abs);
  ...
  for (const ref of hookRefs) {
    const abs = pathApi.resolve(homeAbs, ...ref.split("/"));
    const rel = pathApi.relative(homeAbs, abs);
    if (rel === "") { skipped.push({ path: ref, reason: "is your entire home directory -- too broad to migrate" }); continue; }
    if (rel.startsWith("..") || pathApi.isAbsolute(rel)) {
      skipped.push({ path: ref, reason: "outside your home directory -- sync can only migrate home-relative paths" });
      continue;
    }
    ...
    const relFwd = rel.split(pathApi.sep).join("/");
    if (kind === "dir") { propose(relFwd, "referenced by a settings.json hook"); continue; }
    ...
    const parent = relFwd.split("/").slice(0, -1).join("/");
    if (parent) propose(parent, `contains ${relFwd.split("/").pop()} (referenced by a settings.json hook)`);
    else propose(relFwd, "referenced by a settings.json hook");
  }
